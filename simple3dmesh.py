...
metre = unit('m')
inner_radius = metre(0.025)
middle_line = metre(0.04) # you should change this but if you set this value to 3.5, that 3d model can be easily broke in case of -1 value. so i recommend this value as 4.
...
for index in range(length):
    rx.append( ( middle_line + metre(data[index] * 0.01) ) * (np.cos( (theta[index]) ) )) # index
    ry.append( ( middle_line + metre(data[index] * 0.01) ) * (np.sin( (theta[index]) ) )) 
    rz.append( metre( 0.005 ) )

    rx.append( ( middle_line + metre(data[index] * 0.01) ) * (np.cos( theta[index] ) )) # index + 1
    ry.append( ( middle_line + metre(data[index] * 0.01) ) * (np.sin( theta[index] ) ))
    rz.append( metre(0) )
